...
class Setup extends Database {
  ...
  public function getCurrent() {
    $sql = "SELECT program FROM currentValues;";
    $stmt = $this->conn->prepare($sql);
		$stmt->execute();
		$result = $stmt->fetchAll();
    if (isset($result[0]) && sizeof($result) > 0) {
      $_SESSION['program'] = $result[0];
      return true;
    } else {
      return false;
    }
  ...
  public function updateCurrent($data) {
    // Declare variables
    $program = htmlentities(filter_var($data['program']), FILTER_SANITIZE_NUMBER_INT);

    // Create SQL statement
    $sql  = "UPDATE currentValues (program) VALUES (:program)";
    $stmt = $this->conn->prepare($sql);
    $stmt->execute(
      array(
        'program' => $program
